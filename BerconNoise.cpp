...
class BerconCurveDlgProcNOISE : public ParamMap2UserDlgProc {
	public:
	BerconCurveDlgProcNOISE(){}
	virtual ~BerconCurveDlgProcNOISE() {}
...
INT_PTR BerconNoiseDlgProc::DlgProc(TimeValue t,IParamMap2 *map,HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam)	{	
	...
	case WM_INITDIALOG: {
		...
		int curIndex;
		map->GetParamBlock()->GetValue(noise_function_type, t, curIndex, intvalid);
		SendMessage(GetDlgItem(hWnd, IDC_NOISE_FUNCTION), CB_SETCURSEL, (WPARAM)curIndex, 0);
		map->GetParamBlock()->GetValue(worley_distance, t, curIndex, intvalid);
		SendMessage(GetDlgItem(hWnd, IDC_WORLEY_DISTANCE), CB_SETCURSEL, (WPARAM)curIndex, 0);
		map->GetParamBlock()->GetValue(fractal_type, t, curIndex, intvalid);
		SendMessage(GetDlgItem(hWnd, IDC_FRACTAL_TYPE), CB_SETCURSEL, (WPARAM)curIndex, 0);
		map->GetParamBlock()->GetValue(uvw_dist, t, curIndex, intvalid);
...
void BerconNoise::EnableStuff() {	
	if (pblock) {
		...
		if (map) {
			...
			pblock->GetValue( use_distortion, 0, useDistortion, ivalid);
			pblock->GetValue( noise_function_type, 0, noiseFunction, ivalid);
			pblock->GetValue( fractal_type, 0, fractalFunction, ivalid);
