...
public:
	...
	{
		//if (berconNoise->curve->GetHWND() != GetDlgItem(hWnd, IDC_CURVE)) CurveCtrl::update(berconNoise->curve, GetDlgItem(hWnd, IDC_CURVE), dynamic_cast<ReferenceMaker*>(berconNoise)); // Force update curve
		switch (msg)
		{
		case WM_INITDIALOG:
			CurveCtrl::init;
...
/*
	...
	curve->RegisterResourceMaker(dynamic_cast<ReferenceTarget*>(this));
...
void BerconNoise::EnableStuff() {	
	if (pblock) {
		...
		if (map) {
			...
			pblock->GetValue( noise_function_type, 0, noiseFunction, Interval(TimeValue(0x80000000), TimeValue(0x7fffffff)));
...
RefResult BerconNoise::NotifyRefChanged(const Interval &/*changeInt*/, RefTargetHandle hTarget, PartID& /*partID*/, RefMessage message, BOOL /*propagate*/) {
	switch (message) {
		case REFMSG_CHANGE:
			ivalid.SetEmpty();			
			if (hTarget == pblock) {
...
//		normal.y = (curve->GetControlCurve(0)->GetValue(sc.CurTime(), Noise::limitedNoise(p+DELTA*M[1], np)) - d) / DELTA;				// For removal of custom curve control
