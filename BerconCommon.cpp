...
//moved here from header file as it resolved a problem with another module being able to reference it (I have no idea why)
static void setSpinnerType(IParamMap2 *map, TimeValue t, int pb_id, int edit_id, int spin_id, int spinnerTypeWorld=1, bool allowNegative=false) {	
	...
	float val;
		map->GetParamBlock()->GetValue(pb_id, t, val, intvalid);
...
void BerconXYZ::seedRandomGen(ShadeContext& sc) {
	...
	if (p_randPar) {
		...
		if (ob && ob->IsParticleSystem()) {
			ParticleObject *obj = static_cast<ParticleObject*>(ob);
