...
static Interval intvalid = Interval(TIME_NegInfinity, TIME_PosInfinity);
...
class BerconXYZDlgProc : public ParamMap2UserDlgProc {
	public:
		...
		INT_PTR DlgProc(TimeValue t,IParamMap2 *map,HWND hWnd,UINT msg,WPARAM wParam,LPARAM IParam) {
			HWND hwndMap;
			...
			switch (msg) {
				...
				case WM_SHOWWINDOW: 
					...
					int curIndex;
					map->GetParamBlock()->GetValue(xyz_map, t, curIndex, intvalid);
					SendMessage(GetDlgItem(hWnd, IDC_TYPE), CB_SETCURSEL, (WPARAM)curIndex, 0);
					map->GetParamBlock()->GetValue(xyz_tile_x, t, curIndex, intvalid);
					SendMessage(GetDlgItem(hWnd, IDC_TIL_X), CB_SETCURSEL, (WPARAM)curIndex, 0);
					map->GetParamBlock()->GetValue(xyz_tile_y, t, curIndex, intvalid);
					SendMessage(GetDlgItem(hWnd, IDC_TIL_Y), CB_SETCURSEL, (WPARAM)curIndex, 0);
					map->GetParamBlock()->GetValue(xyz_tile_z, t, curIndex, intvalid);
