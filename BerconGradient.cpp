...
void BerconGradient::keyNumChanged(int num)
{
	...
	Update(GetCOREInterface()->GetTime(), intkeyn);
}


void BerconGradient::keyPosChanged(float pos) {
	static Interval intkeypc = Interval();
	if (!pblock) return;
	if (gradient->selected >= 0 && gradient->selected < gradient->numKeys()) {
		TimeValue t = GetCOREInterface()->GetTime();		
		pblock->SetValue(pb_positions, t, pos, gradient->selected);
		Update(GetCOREInterface()->GetTime(),intkeypc);
	}
}
...
void BerconGradient::gradMoveKey(int n, float pos) {
	static Interval intkeygm = Interval();
	TimeValue t = GetCOREInterface()->GetTime();
	if (n < countKeys())
		pblock->SetValue(pb_positions, t, pos, n);
	ivalid.SetEmpty();
	Update(GetCOREInterface()->GetTime(),intkeygm);
}


void BerconGradient::gradAddKey(float pos) {
	static Interval intkeyga = Interval();
	AColor* col = &gradient->getColor(pos);			// this makes absolutely no sense to me, What is this supposed to do??
	Texmap* sub = NULL;
	pblock->Append(pb_submaps, 1, &sub);
	pblock->Append(pb_colors, 1, &col);
	pblock->Append(pb_positions, 1, &pos);
	ivalid.SetEmpty();
	Update(GetCOREInterface()->GetTime(),intkeyga);
}

void BerconGradient::gradDelKey(int n) {
	static Interval intkeydk = Interval();
	if (!pblock) return;
	...
	ivalid.SetEmpty();
	Update(GetCOREInterface()->GetTime(),intkeydk);
}
...
void BerconGradient::gradSelKey() {
	static Interval intkeygsk = Interval();
	if (!pblock) return;
	ivalid.SetEmpty();
	Update(GetCOREInterface()->GetTime(),intkeygsk);		
...
// Output: -1..1
/* occCurve does not appear to be used?
