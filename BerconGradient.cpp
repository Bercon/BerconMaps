...
class BerconCurveDlgProcGRADIENT : public ParamMap2UserDlgProc {
	public:
		...
		virtual INT_PTR DlgProc(TimeValue t,IParamMap2 *map,HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam) {
			if (parentMap->cc_curve->GetHWND() != GetDlgItem(hWnd, IDC_CURVE))
				CurveCtrl::update(parentMap->cc_curve, GetDlgItem(hWnd, IDC_CURVE), static_cast<ReferenceMaker*>(parentMap)); // Force update cc_curve
			switch (msg) {
				case WM_INITDIALOG:
				case WM_SHOWWINDOW:
					CurveCtrl::update(parentMap->cc_curve, GetDlgItem(hWnd, IDC_CURVE), static_cast<ReferenceMaker*>(parentMap));					
					break;
				case WM_DESTROY:
					CurveCtrl::disable(parentMap->cc_curve);
					break;
		...
		void SetThing(ReferenceTarget *m) { 
			CurveCtrl::disable(parentMap->cc_curve); // Disable previously used cc_curve
...
#else
	cc_curve->RegisterResourceMaker(static_cast<ReferenceMaker*>(this));
#endif
...
float occ_curve(float occ, float val) {					// Does this do something?
...
AColor BerconGradient::EvalColor(ShadeContext& sc) {
	...
	// Curve
	if (p_curveOn) d = cc_curve->GetControlCurve(0)->GetValue(sc.CurTime(), d);		
...
Point3 BerconGradient::EvalNormalPerturb(ShadeContext& sc) {
	...
	/*if (berconXYZ.req()) {		
		...
		for (int i=0; i<3; i++) {
			...
			if (p_curveOn) 
				normal[i] = cc_curve->GetControlCurve(0)->GetValue(sc.CurTime(), normal[i]);
