...
INT_PTR BerconWoodDlgProc::DlgProc(TimeValue t,IParamMap2 *map,HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam)	{	
	switch (msg) {
		case WM_INITDIALOG:	{			
			...
			int curIndex;
			map->GetParamBlock()->GetValue(wood_type, t, curIndex, intvalid);
...
void BerconWood::EnableStuff() {
	if (pblock) {
		...
		if (map) {
			...
			pblock->GetValue( use_distortion, 0, useDistortion, intvalid);
			pblock->GetValue( lock_grain, 0, lockGrain, intvalid);		
