...
class BerconCurveDlgProc : public ParamMap2UserDlgProc {
	...
	virtual INT_PTR DlgProc(TimeValue t,IParamMap2 *map,HWND hWnd,UINT msg,WPARAM wParam,LPARAM lParam) {
			if (berconWood->cc_curve->GetHWND() != GetDlgItem(hWnd, IDC_CURVE))
				CurveCtrl::update(berconWood->cc_curve, GetDlgItem(hWnd, IDC_CURVE), static_cast<ReferenceMaker*>(berconWood)); // Force update cc_curve
			switch (msg) {
				case WM_INITDIALOG:
				case WM_SHOWWINDOW:
					CurveCtrl::update(berconWood->cc_curve, GetDlgItem(hWnd, IDC_CURVE), static_cast<ReferenceMaker*>(berconWood));					
					break;
				case WM_DESTROY:
					CurveCtrl::disable(berconWood->cc_curve);
					break;
		...
		void SetThing(ReferenceTarget *m) { 
			CurveCtrl::disable(berconWood->cc_curve); // Disable previously used cc_curve
...
void BerconWood::Reset() {
	...
	cc_curve = (ICurveCtl *) CreateInstance(REF_MAKER_CLASS_ID,CURVE_CONTROL_CLASS_ID);
	cc_curve->RegisterResourceMaker(static_cast<ReferenceTarget*>(this));
...
void BerconWood::SetReference(int i, RefTargetHandle rtarg) {	
	switch(i) {
		...
		case CURVE_REF: cc_curve = (ICurveCtl *)rtarg; break;
...
void BerconWood::applyDistortion(ShadeContext& sc_, Point3& p) {
	if (subtex[3])
		if (subtex[4])
			p += subtex[3]->EvalNormalPerturb(sc_)*distortionStr*subtex[4]->EvalMono(sc_);
		else					
			p += subtex[3]->EvalNormalPerturb(sc_)*distortionStr;
}

WoodParam BerconWood::EvalParameters(ShadeContext& sc_) {
	WoodParam wp;
	...
	// Use maps
	if (mappedParameters) {	
		wp.lowTresh = mapOn[6]&&subtex[6]?subtex[6]->EvalMono(sc_)*lowTresh:lowTresh;
		wp.highTresh = mapOn[7]&&subtex[7]?subtex[7]->EvalMono(sc_)*highTresh:highTresh;
		wp.skew = mapOn[8]&&subtex[8]?subtex[8]->EvalMono(sc_)*skew:skew;
		wp.widthVar = mapOn[9]&&subtex[9]?subtex[9]->EvalMono(sc_)*widthVar:widthVar;
		wp.gainVar = mapOn[10]&&subtex[10]?subtex[10]->EvalMono(sc_)*gainVar:gainVar;
		wp.trunkStr = mapOn[11]&&subtex[11]?subtex[11]->EvalMono(sc_)*trunkStr:trunkStr;
		wp.trunkFreq = mapOn[12]&&subtex[12]?subtex[12]->EvalMono(sc_)*trunkFreq:trunkFreq;
		wp.radialStr = mapOn[13]&&subtex[13]?subtex[13]->EvalMono(sc_)*radialStr:radialStr;
		wp.radialFreq = mapOn[14]&&subtex[14]?subtex[14]->EvalMono(sc_)*radialFreq:radialFreq;
		wp.radialZ = mapOn[15]&&subtex[15]?subtex[15]->EvalMono(sc_)*radialZ:radialZ;
		wp.angleStr = mapOn[16]&&subtex[16]?subtex[16]->EvalMono(sc_)*angleStr:angleStr;
		wp.angleFreq = mapOn[17]&&subtex[17]?subtex[17]->EvalMono(sc_)*angleFreq:angleFreq;
		wp.angleRad = mapOn[18]&&subtex[18]?subtex[18]->EvalMono(sc_)*angleRad:angleRad;
	} else {
...
AColor BerconWood::EvalColor(ShadeContext& sc_) {
	Point3 p,dpdx,dpdy;
	if (!sc_.doMaps) return black;

	// If we've already evalutated the color at this point we'll use it and stop here
	AColor c;
	if (sc_.GetCache(this,c)) 
		return c; 	
	if (gbufID) sc_.SetGBufferID(gbufID);

	// Evaluate parameters
	WoodParam wp = EvalParameters(sc_);
	float grainA = mapOn[19]&&subtex[19]?subtex[19]->EvalMono(sc_)*grainAmount:grainAmount;
	float grainF = mapOn[20]&&subtex[20]?subtex[20]->EvalMono(sc_)*grainFreq:grainFreq;

	// UVW, Distortion and size
	berconXYZ.get(sc_,p,dpdx,dpdy);
	if (useDistortion)
		applyDistortion(sc_,p);	
	float wSize = mapOn[5]&&subtex[5]?subtex[5]->EvalMono(sc_)*woodSize:woodSize;
	p /= wSize; dpdx /= (wSize / 2.f); dpdy /= (wSize / 2.f);
		
	// Caluclate wood function and grain
	Point3 gP;
	
	float d = sc_.filterMaps? Noise::wood(p, dpdx, dpdy, gP, wp) : Noise::wood(p, gP, wp);

	float g = (grainAmount > .001f) ? Fractal::grain(gP, grainA, grainF): 0.f;	

	// Get colors
	RGBA c0 = mapOn[0]&&subtex[0] ? subtex[0]->EvalColor(sc_): col[0];
	RGBA c1 = mapOn[1]&&subtex[1] ? subtex[1]->EvalColor(sc_): col[1];
	RGBA c2 = lockGrain ? c1: (mapOn[2]&&subtex[2] ? subtex[2]->EvalColor(sc_): col[2]);


	// Apply curves
	if (useCurve)
		d = cc_curve->GetControlCurve(0)->GetValue(sc_.CurTime(), d);
...
float BerconWood::EvalMono(ShadeContext& sc_) {
	return Intens(EvalColor(sc_));
}

#define DELTA 0.001f
Point3 BerconWood::EvalNormalPerturb(ShadeContext& sc_) {	
	Point3 p,dpdx,dpdy;
	
	if (!sc_.doMaps) return Point3(0,0,0);
	if (gbufID) sc_.SetGBufferID(gbufID);
	...
	// UVW, Distortion and size
	Point3 M[3];
	berconXYZ.get(sc_, p, dpdx, dpdy, M);
	...
	float d = sc_.filterMaps? Noise::wood(p, dpdx, dpdy, gP, wp) : Noise::wood(p, gP, wp);
	if (useCurve) d = cc_curve->GetControlCurve(0)->GetValue(sc_.CurTime(), d);
	
	float g = grainON ? Fractal::grain(gP, grainA, grainF): 0.f;	
	
	for (int i=0; i<3; i++) {		
		np[i] = sc_.filterMaps? Noise::wood(p + DELTA * M[i], dpdx, dpdy, gP, wp) : Noise::wood(p + DELTA * M[i], gP, wp);
		if (useCurve) np[i] = cc_curve->GetControlCurve(0)->GetValue(sc_.CurTime(), np[i]);	
		np[i] = (np[i] - d) / DELTA;
		if (grainON) nG[i] = Fractal::grain(gP, grainA, grainF); // gP is updated by wood()
	}

	np = -sc_.VectorFromNoScale(np, REF_OBJECT);
	nG = -sc_.VectorFromNoScale(nG, REF_OBJECT);
	...
	bool maps = false;
	if (subtex[0]) {
		f1 = subtex[0]->EvalMono(sc_);
		v1 = subtex[0]->EvalNormalPerturb(sc_);
		maps = true;
	} else {
		f1 = Intens(col[0]);
	...
	if (subtex[1]) {
		f2 = subtex[1]->EvalMono(sc_);
		v2 = subtex[1]->EvalNormalPerturb(sc_);
		maps = true;
	} else {
		f2 = Intens(col[1]);
	...
	if (subtex[2]) {
		f3 = subtex[2]->EvalMono(sc_);
		v3 = subtex[2]->EvalNormalPerturb(sc_);
		maps = true;
	} else {
		f3 = Intens(col[2]);
